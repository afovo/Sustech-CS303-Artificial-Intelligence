...
# if [x][y] has my piece: (my & (1 << (x * 8 + y))) != 0
# if [x][y] has op piece: (op & (1 << (x * 8 + y))) != 0
...
def get_candidate_list(color, pieces_b, pieces_w):
    ...
    for i in range(8):
        for j in range(8):
            if (my & (1 << (i * 8 + j))) ^ (op & (1 << (i * 8 + j))) == 0:  # one available position
                for dx, dy in direction:
                    ...
                    while 0 <= x <= 7 and 0 <= y <= 7 and (op & (1 << (x * 8 + y))) != 0:
                        ...
                        y += dy
                        flag = True
                    if flag and 0 <= x <= 7 and 0 <= y <= 7 and (my & (1 << (x * 8 + y))) != 0:
...
class AI(object):
    ...
    def h_alphabeta_search(self, pieces_b, pieces_w, cutoff_depth):

        def max_value(pieces_b, pieces_w, alpha, beta, depth):
            moves = get_candidate_list(self.color, pieces_b, pieces_w)
            if depth > cutoff_depth or not moves:
                return evaluation_func(self.color, pieces_b, pieces_w, depth + self.round, len(moves)), None

            v, move = -infinity, None
            for m in moves:
                new_b, new_w = move_one_step(m, self.color, pieces_b, pieces_w)
        ...
        def min_value(pieces_b, pieces_w, alpha, beta, depth):
            moves = get_candidate_list(self.op_color, pieces_b, pieces_w)
            if depth > cutoff_depth or not moves:
                return evaluation_func(self.op_color, pieces_b, pieces_w, depth + self.round, len(moves)), None

            v, move = +infinity, None
            for m in moves:
                new_b, new_w = move_one_step(m, self.op_color, pieces_b, pieces_w)
    ...
    def go(self, chessboard):
        ...
        self.candidate_list = get_candidate_list(self.color, int(pieces_b), int(pieces_w))
        # call search to get the best one and append it
        _, move = self.h_alphabeta_search(pieces_b, pieces_w, 3)
        if move is not None:
